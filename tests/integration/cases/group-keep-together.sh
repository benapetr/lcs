...
vip_owner_has()
{
    ...
    local vip="$2"
    local owner="$3"
    status_text "$node" 2>/dev/null | grep -Fq "$vip " &&
        status_text "$node" 2>/dev/null | grep -Fq "$vip " &&
...
status_text node1 | grep -Fq "vip2 127.0.0.202/32 dev=lo state=active owner=node1" || die "vip2 not active on node1"
...
log "group keep-together regression passed"
