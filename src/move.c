    ...
    if (move->resource_idx >= 0 && move->target_idx >= 0)
    {
        ...
        if (move->final_status == 0)
        {
                ...
                !res->home_blocked)
            {
                ...
                lcs_log_info("manual move of resource %s to %s blocked home-node rebalance to %s",
                             resource->name,
            ...
            lcs_log_info("%s of resource %s to %s completed: %s",
                         origin, g_state.cfg.resources[move->resource_idx].name,
                         cluster_node_name_or_none(move->target_idx),
                         move->final_message);
        } else
        {
            lcs_log_warn("%s of resource %s to %s failed: %s",
                         origin, g_state.cfg.resources[move->resource_idx].name,
                         cluster_node_name_or_none(move->target_idx),
                         move->final_message);
        }
...
static int move_validate_request(const void *payload, uint32_t len, int *resource_idx, int *target_idx, char *message, size_t message_len)
{
    ...
    *resource_idx = lcs_config_resource_index(&g_state.cfg, vip_name);
    *target_idx = lcs_config_node_index(&g_state.cfg, target_name);
    if (*resource_idx < 0 || *target_idx < 0)
    {
    ...
    if (g_state.resources[*resource_idx].disabled)
    {
...
static void move_start_target(int epoll_fd, move_runtime_t *move)
{
    ...
    if (move->old_owner_idx >= 0 && move->old_owner_idx != g_state.self_index)
    {
        ...
        lcs_log_info("old owner %s is offline for resource %s; waiting for lease expiry",
                     cluster_node_name_or_none(move->old_owner_idx),
                     g_state.cfg.resources[move->resource_idx].name);
        move->phase = LCS_MOVE_PHASE_WAIT_OLD_LEASE_EXPIRY;
        return;
...
static int move_start_remote_target(int epoll_fd, int cli_server_slot, uint32_t cli_server_seq, const void *payload, uint32_t len, int resource_idx, int target_idx)
{
                       ...
                       timeout_ms, move_peer_callback, &move->rpc_ctx[0]) != 0)
    {
        if (!move->active)
            return -1;
        if (g_state.resources[resource_idx].owner_node == target_idx &&
            g_state.resources[resource_idx].state == LCS_RES_ACTIVE)
        {
            move->final_status = 0;
...
bool move_active_for_resource(int resource_idx)
{
    for (size_t i = 0; i < LCS_MOVE_OP_MAX; i++)
    {
        if (g_state.moves[i].active && g_state.moves[i].resource_idx == resource_idx)
            return true;
    }
    return false;
}
...
int move_start_internal(int epoll_fd, int resource_idx, int target_idx, const char *reason)
{
    ...
    if (!cluster_has_quorum() ||
        ...
        move_active_for_resource(resource_idx) ||
        lease_operation_active(resource_idx))
        return -1;
...
int move_start_cli_server(int epoll_fd, int cli_server_slot, uint32_t cli_server_seq, const void *payload, uint32_t len)
{
    ...
    if (anchor_idx >= 0 && anchor_idx != resource_idx)
    {
        lcs_log_info("move request for resource %s redirected to keep-together anchor resource %s",
                     g_state.cfg.resources[resource_idx].name,
                     g_state.cfg.resources[anchor_idx].name);
        resource_idx = anchor_idx;
    }
...
static void move_process_target(move_runtime_t *move, int epoll_fd, uint64_t now)
{
    if (move->phase == LCS_MOVE_PHASE_WAIT_OWNER_RELEASE)
    {
        ...
        lcs_log_info("old owner %s did not confirm release of resource %s; waiting for lease expiry",
                     cluster_node_name_or_none(move->old_owner_idx),
                     g_state.cfg.resources[move->resource_idx].name);
        move->phase = LCS_MOVE_PHASE_WAIT_OLD_LEASE_EXPIRY;
    }
...
static void move_process_lease(move_runtime_t *move, int epoll_fd)
{
    for (size_t i = 0; i < g_state.cfg.node_count; i++)
    {
            ...
            status == 0)
        {
            ...
            move->votes++;
            lcs_log_debug("lease request for resource %s epoch=%llu acked by %s",
                          g_state.cfg.resources[move->resource_idx].name,
...
void move_process(int epoll_fd)
{
    ...
    for (size_t i = 0; i < LCS_MOVE_OP_MAX; i++)
    {
        ...
        if (move->phase == LCS_MOVE_PHASE_WAIT_TARGET)
        {
            ...
            } else if (move->resource_idx >= 0 &&
                       g_state.resources[move->resource_idx].owner_node == move->target_idx &&
                       g_state.resources[move->resource_idx].state == LCS_RES_ACTIVE)
            {
                move->final_status = 0;
                snprintf(move->final_message, sizeof(move->final_message), "move completed");
            } else
